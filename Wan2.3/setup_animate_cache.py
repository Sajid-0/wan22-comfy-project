#!/usr/bin/env python3
"""
Wan2.2 Animate-14B Model Download and Cache Setup Script for RunPod GPU
...
class WanAnimateCacheManager:
    def __init__(self, cache_dir="/home/caches", workspace_dir="/workspace/wan22-comfy-project"):
        ...
        self.models = {
            "animate-14b": {
                ...
                "cache_path": self.cache_dir / "Wan2.2-Animate-14B",
                "required_files": [
                    # Main DiT model files
                    ...
                    "xlm-roberta-large/pytorch_model.bin",  # CLIP tokenizer weights
                    # VAE
                    "Wan2.1_VAE.pth",
                    # Configs
                    "config.json",
                    ...
                    # Preprocessing checkpoint directory exists
                    "process_checkpoint/sam2.1_hiera_large.pt",
                ]
            }
        }
    ...
    def setup_cache_directories(self):
        ...
        for model_key, model_info in self.models.items():
            ...
            # Create subdirectories for Animate-specific components
            (model_info["cache_path"] / "process_checkpoint").mkdir(parents=True, exist_ok=True)
            (model_info["cache_path"] / "xlm-roberta-large").mkdir(parents=True, exist_ok=True)
    ...
    def check_model_integrity(self, model_key):
        ...
        if missing_files:
            # Show only first few missing files to avoid clutter
            sample_missing = missing_files[:3]
            logger.warning(f"🔍 Integrity check: Missing {len(missing_files)} files for {model_info['name']}, e.g.: {sample_missing}")
            return False
    ...
    def download_model(self, model_key, force_redownload=False):
        ...
        logger.info(f"    This is a large model (~50GB+). Please be patient...")
        ...
        try:
            ...
            snapshot_download(
                ...
                allow_patterns=["*.json", "*.safetensors", "*.pth", "*.pt", "*.bin"], # All model files
            ...
            if self.check_model_integrity(model_key):
                ...
                logger.warning("⚠️  Download completed, but some files may be missing.")
                logger.info("    This is normal if preprocessing checkpoints weren't fully downloaded.")
                logger.info("    Main model files should be ready to use.")
                return True  # Return True anyway since core files are likely present
    ...
    def run_full_setup(self):
        ...
        logger.info("🚀 Starting full Wan2.2 Animate setup for RunPod...")
        logger.info("📝 Note: This will download ~50GB+ of model files to /home/caches")
        ...
        if all_models_downloaded:
            ...
            logger.info("💡 Ready to run: python run_animate_multi_gpu.py")
            logger.info("⚠️  Note: You must preprocess your video first!")
            logger.info("   See wan/modules/animate/preprocess/UserGuider.md for details")
        else:
    ...
    def quick_check_and_setup(self):
        ...
        logger.info("⚡ Quick setup check for Animate models...")
    ...
    def display_status(self):
        ...
        for model_key, model_info in self.models.items():
            # For Animate, just check if main files exist
            cache_path = model_info["cache_path"]
            has_main_files = (cache_path / "diffusion_pytorch_model-00001-of-00004.safetensors").exists()
            status = "✅ Complete" if has_main_files else "❌ Incomplete"
        ...
        has_models = all(
            (self.models[key]["cache_path"] / "diffusion_pytorch_model-00001-of-00004.safetensors").exists()
        ...
        if has_models:
            ...
            logger.info("   python run_animate_multi_gpu.py")
            logger.info("\n⚠️  IMPORTANT: You must preprocess your video first!")
            logger.info("   python wan/modules/animate/preprocess/preprocess_data.py \\")
            logger.info("     --ckpt_path ./Wan2.2-Animate-14B/process_checkpoint \\")
            logger.info("     --video_path <your_video.mp4> \\")
            logger.info("     --refer_path <character_image.jpg> \\")
            logger.info("     --save_path ./processed_output \\")
            logger.info("     --resolution_area 1280 720")
        else:
            logger.info("⚠️  Some models are missing. Run setup or download to get them.")

def main():
    parser = argparse.ArgumentParser(description="Wan2.2 Animate Cache and Model Manager for RunPod")
    ...
    manager = WanAnimateCacheManager()

    try:
        if args.action == "status":
            manager.display_status()
        elif args.action == "download":
            manager.download_model("animate-14b", force_redownload=args.force_redownload)
            manager.display_status()
            ...
            print("\n" + "="*50)
            print("    Wan2.2 Animate Cache Manager for RunPod")
            print("="*50)
            print("\nRunPod Note: /home is volatile and resets on instance restart")
            print("This script will auto-download models when needed (~50GB+)")
            print("\nChoose an option:")
            ...
            if choice == "1": 
                ...
                success = manager.download_model("animate-14b", force_redownload=force)
                manager.display_status()
                sys.exit(0 if success else 1)
            elif choice == "4": 
                manager.display_status()
