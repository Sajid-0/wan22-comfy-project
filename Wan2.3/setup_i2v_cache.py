#!/usr/bin/env python3
"""
Wan2.2 I2V Model Download and Cache Setup Script for RunPod GPU
...
class WanI2VCacheManager:
    def __init__(self, cache_dir="/home/caches", workspace_dir="/workspace/wan22-comfy-project"):
        ...
        self.models = {
            "i2v-a14b": {
                ...
                "cache_path": self.cache_dir / "Wan2.2-I2V-A14B",
                "required_files": [
                    # High noise model files
                    "high_noise_model/diffusion_pytorch_model-00001-of-00004.safetensors",
                    "high_noise_model/diffusion_pytorch_model-00002-of-00004.safetensors", 
                    "high_noise_model/diffusion_pytorch_model-00003-of-00004.safetensors",
                    "high_noise_model/diffusion_pytorch_model-00004-of-00004.safetensors",
                    "high_noise_model/diffusion_pytorch_model.safetensors.index.json",
                    "high_noise_model/config.json",
                    # Low noise model files
                    "low_noise_model/diffusion_pytorch_model-00001-of-00004.safetensors",
                    "low_noise_model/diffusion_pytorch_model-00002-of-00004.safetensors", 
                    "low_noise_model/diffusion_pytorch_model-00003-of-00004.safetensors",
                    "low_noise_model/diffusion_pytorch_model-00004-of-00004.safetensors",
    ...
    def setup_cache_directories(self):
        ...
        for model_key, model_info in self.models.items():
            ...
            # Create subdirectories for MoE models
            (model_info["cache_path"] / "high_noise_model").mkdir(parents=True, exist_ok=True)
            (model_info["cache_path"] / "low_noise_model").mkdir(parents=True, exist_ok=True)
    ...
    def check_model_integrity(self, model_key):
        ...
        if missing_files:
            logger.warning(f"🔍 Integrity check: Missing files for {model_info['name']}: {missing_files[:5]}...")
            return False
    ...
    def download_model(self, model_key, force_redownload=False):
        ...
        logger.info(f"    This is a large model (~54GB). Please be patient...")
    ...
    def run_full_setup(self):
        ...
        logger.info("🚀 Starting full Wan2.2 I2V setup for RunPod...")
        logger.info("📝 Note: This will download ~54GB of model files to /home/caches")
        ...
        if all_models_downloaded:
            ...
            logger.info("💡 Ready to run: python run_i2v_multi_gpu.py")
        else:
    ...
    def quick_check_and_setup(self):
        ...
        logger.info("⚡ Quick setup check for I2V models...")
    ...
    def display_status(self):
        ...
        if all(self.check_model_integrity(key) for key in self.models.keys()):
            ...
            logger.info("   python run_i2v_multi_gpu.py")
        else:
            logger.info("⚠️  Some models are missing. Run setup or download to get them.")

def main():
    parser = argparse.ArgumentParser(description="Wan2.2 I2V Cache and Model Manager for RunPod")
    ...
    manager = WanI2VCacheManager()

    try:
        if args.action == "status":
            manager.display_status()
        elif args.action == "download":
            manager.download_model("i2v-a14b", force_redownload=args.force_redownload)
            manager.display_status()
            ...
            print("\n" + "="*50)
            print("    Wan2.2 I2V Cache Manager for RunPod")
            print("="*50)
            print("\nRunPod Note: /home is volatile and resets on instance restart")
            print("This script will auto-download models when needed (~54GB)")
            print("\nChoose an option:")
            ...
            if choice == "1": 
                ...
                success = manager.download_model("i2v-a14b", force_redownload=force)
                manager.display_status()
                sys.exit(0 if success else 1)
            elif choice == "4": 
                manager.display_status()
